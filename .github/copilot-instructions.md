...
- DO: For session-scoped confirm/apply/save steps use the shared helpers in `qapbot/ui_common.py`: `if not await claim_action(self, interaction): return` as the FIRST statement (a rejected re-click is ack'd silently); `release_action(self)` on an early bail-out before the action starts (e.g. failed permission check); `await lock_buttons(self, interaction, content=...)` as the click's first response (greys every component out, optional "processing…" text) instead of `defer()`; `await unlock_buttons(self, interaction)` for dialogs that stay open afterwards. Cancel/No buttons claim the view too, so they can't overwrite a running confirm. `tests/discord/test_ui_double_click_guards.py` lists every guarded handler structurally — add new confirm steps there.
...
- Clickable slash command in message text → `command_mention("registration")` / `command_mention("cwl preferences")` (`QBdiscocmdshelper.py`), passed into the text as a `t()` placeholder (e.g. `{registration}`), never a hardcoded `` `/cmd` ``. It renders `</name:id>` from the ids the startup command sync returned (`CACHE.app_command_ids`), with plain `` `/name` `` as fallback. `/help` uses the same helper.
- War file lifecycle → see ../qapbot/docs/WAR_FILE_MANAGEMENT_RULES.md
- Custom emoji/icon → `BotEmojis.X` (`qapbot/emojis.py`), never a hardcoded `<:name:id>`. Every icon is an **application emoji** the bot owns and uploads itself: master art in `qapbot/icons/` (SVG for vector icons — also what the Activity bundles; raster masters as-is), one entry in `qapbot/icons/emoji_manifest.json`, one fallback line in `BotEmojis`, then `npm run build:emoji` in `activity/client` to generate the lossless square WebP into `qapbot/icons/emoji/` (commit it). The bot uploads missing ones on its next start. In a Button use `emoji=`, never the label text — a custom emoji only renders there. Tests enforce manifest/`BotEmojis` parity and the WebP format. 📖 ../plans/implemented/app-emoji-migration.md
