...
from clashcontrol.exceptions import ValidationError
...
# test-dependent moment — clashcontrol.QBdiscocmdshelper_cwl.py reads CACHE via its own module-level
...
async def complete_account_linking_flow(
    *,
    ...
    if existing_owner and not api_token and not admin_override:
        ...
        from clashcontrol.ui_registration import ApiTokenOwnershipModal
        from clashcontrol.i18n import t
        guild_id = guild.id if guild else None
    ...
    if api_token_override:
        ...
        from clashcontrol.ui_registration import ApiVerificationPromptView
        from clashcontrol.i18n import t
        guild_id = guild.id if guild else None
...
def get_playerregistration_message(server_name: str, guild_id: Optional[int] = None) -> str:
    ...
    """
    from clashcontrol.i18n import t, get_guild_language
    import logging
...
async def _format_clan_management_registrations(clan_tag: str, guild: discord.Guild) -> Tuple[discord.Embed, Optional[discord.Embed], List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    from clashcontrol.formatting import best_practice_player_cell  # noqa: F401
