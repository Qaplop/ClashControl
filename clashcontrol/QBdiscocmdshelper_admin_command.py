...
async def handle_cleanup_messages_channel(interaction: Any, channel: Any, bot: Any, username: str) -> str:
    ...
    """
    import logging
    from clashcontrol.QBdiscocmdshelper import cleanup_channel_messages
...
def _build_cache_summary(cache: Any) -> list[str]:
    ...
    try:
        ...
        from clashcontrol.cache_manager import MAX_TEMP_WAR_OBJECTS as _mwo_cap
...
def _get_rss_breakdown() -> dict[str, Any]:
    ...
    clashcontrol/db_manager.py mmap's the DB via `db_memory_pragmas()` (main 1 GB, history 256 MB as
...
async def handle_backfill_cwl_groups(cache: Any, season: str) -> str:
    ...
    After this runs, execute::
        ...
        python clashcontrol/scripts/analyze_cwl_rounds.py <season> --fetch-missing
...
def _explicit_cols(conn: Any, table: str) -> str:
    ...
    WarHistoryDB._explicit_column_list_sync's docstring (clashcontrol/db_manager.py) for why every
