...
from clashcontrol.constants import SECONDS_PER_HOUR, WAR_UPDATE_LEAGUES, normalize_cwl_season
...
class CacheManager:
    ...
    def __init__(self) -> None:
        ...
        # Updated by load_all_temp_war_stats() and the ClashControl.py cycle pre-scan.
        ...
        # command sync (ClashControl._setup_hook). Lets message text contain clickable </name:id>
        ...
        # ClashControl.main() and read at the end for the [CYCLE-SUMMARY] log line.
    ...
    def evict_stale_player_cache(self) -> None:
        """Purge _player_cache entries past their TTL. Called once per update cycle (ClashControl.py),
    ...
    def get_war_file_stats(self) -> Dict[str, int]:
        ...
        # can race at the bytecode level. Same snapshot idiom ClashControl.py's own cycle code already
    ...
    async def get_capital_raid_seasons_from_api(self, clan_tag: str, *, limit: int = 1) -> Dict[str, Any]:
        ...
        free while automatic GC is off (clashcontrol/docs/PERFORMANCE_TUNING.md). Only ``items[0]``
    ...
    async def _sync_group_track_war_updates(self, clan_objs: List[Any], league_rank: str, cwl_season: str) -> None:
        ...
        promotion/demotion (clashcontrol/coc_cache.py) and ``_harvest_cwl_group_clans()``'s
        ...
        clashcontrol/scripts/repromote_mid_season_clans.py, written after discovering
        ...
        later in the same season — ``CWL-GROUP-EXPAND`` (``ClashControl.py``)
        ...
        ``clashcontrol/docs/CWL_ROUND_TRACKING_PLAN.md`` 2026-08-09 finding:
