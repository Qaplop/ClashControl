...
/bug, /feature (QBdiscordcmds.py)
        │
        ▼
clashcontrol/ui_tracker.py        ── all Discord UI + item lifecycle logic
        │
        ▼
CACHE.db_manager (db_manager.py)   ── bot_settings / tracker_items / tracker_attachments /
        ...
        │
        ▼
clashcontrol/web_bridge.py  /api/tracker/*   ── HTTP surface for the agent side, X-Bridge-Secret gated
        │
        ▼
clashcontrol/mcp/tracker_mcp.py   ── stdio MCP server, 12 tools, used by Copilot Chat / Claude Code
```

Only PROD ever owns the tracker: `CONFIG.tracker_enabled` is hard-coded to `not is_dev_mode`
in `clashcontrol/config.py`'s `load_config()` — there is no env var, and DEV can never register
...
`clashcontrol/docs/CODE_STRUCTURE.md`'s "Text Input Labels" section) — ticket #0105 was a live crash
...
`ClashControl.py`'s `on_member_join`, finishes the grant automatically once the reporter actually joins
...
## Discord surface (`clashcontrol/ui_tracker.py`)
...
The module shadows `clashcontrol.i18n.t` right after its import (`t = ` wrapper defined immediately
below `from clashcontrol.i18n import t as _t_localized`) with a version that drops any
`guild_id`/`user_id` kwargs before delegating — every remaining `t('ui_components.tracker...',
guild_id=..., user_id=...)` call site in this file keeps working unchanged, but `clashcontrol.i18n.t()`
...
`clashcontrol/docs/COPILOT_PITFALLS_COOKBOOK.md`, for the full writeup — apply the same pattern to any
...
- Attachments: `attachment1..3` command params are pre-downloaded as a background task
  **immediately after** `send_modal()` — the signed CDN URL on the user's own attachment
  ...
  wired into `ClashControl.py`'s existing `on_message` handler *before* its DM-fallback branch) —
...
- `TrackerItemButton` / `TrackerTestPassButton` / `TrackerTestFailButton` / `TrackerTestMoveDoneButton`
  ...
  in `ClashControl.py`'s `_setup_hook()`. Each implements `interaction_check()` gating on
...
- The 👍-reaction sign-off shortcut is a **new** `on_raw_reaction_add` listener in `ClashControl.py`
...
`check_bot_admin_or_tester()` (`clashcontrol/QBdiscocmdshelper.py`) — the bot admin **or** anyone in
...
`clashcontrol/docs/COPILOT_PITFALLS_COOKBOOK.md`). `ConfirmForceMoveView._on_yes` shares its result text
...
`CwlCarryOverPromptView._finish()` (`clashcontrol/ui_cwl_roster.py`) is a known, unfixable exception to
...
### Bridge (`clashcontrol/web_bridge.py`, `/api/tracker/*`)
...
All handlers delegate the actual DB+Discord work to `clashcontrol/ui_tracker.py`
...
### MCP server (`clashcontrol/mcp/tracker_mcp.py`)

Stdio server, launched as `python -m clashcontrol.mcp.tracker_mcp` by both `.mcp.json` (Claude Code)
...
**Untrusted input (`clashcontrol/mcp/tracker_envelope.py`)**: bug/feature reports are arbitrary text
...
- **MCP setup**: set `TRACKER_BRIDGE_URL`/`TRACKER_BRIDGE_SECRET`/`TRACKER_ADMIN_ID` in `.env`.
  ...
  `clashcontrol/config.py`), so `.env` alone is sufficient — never put these as literals in
...
- `clashcontrol/docs/DATABASE_ARCHITECTURE.md` § Bug/Feature Tracker — table definitions.
- `clashcontrol/docs/COPILOT_PITFALLS_COOKBOOK.md` — the expiring-CDN-attachment-URL pitfall.
