...
**Location**: [clashcontrol/db_manager.py](../db_manager.py#L1453)
...
### 1. CoCClanCache (clashcontrol/coc_cache.py)
...
**Location**: [clashcontrol/cache_manager.py](../cache_manager.py#L2832)
...
**Location**: [ClashControl.py](../../ClashControl.py#L543-L949) (categorization starts in `async def main()`
...
`compute_recheck_offset_seconds(clan_tag)` (ClashControl.py, module level) adds a stable
...
- `ClashControl.py`'s subscription loop, only for clans with a `cwlgroup` PNG subscription.
...
2. `cwl_season_window_closed()` (`clashcontrol/constants.py`) — the season's war window is definitively
...
### Exception Hierarchy
**Location**: [clashcontrol/exceptions.py](../exceptions.py#L19)
...
- Caught in Phase-1 loop (ClashControl.py#L904-L929, inside `fetch_single_clan()`) and bucketed into cycle stats
...
# _WAR_UPDATE_LEAGUES: single source of truth clashcontrol/constants.py:WAR_UPDATE_LEAGUES
...
**Location**: [clashcontrol/cache_manager.py](../cache_manager.py#L2107)

Writes modified clan_name_cache entry to DB via `db_manager.save_clan()`.

### db_manager.save_clan() / _save_clan_unlocked()
**Location**: [clashcontrol/db_manager.py](../db_manager.py#L3699) / [../db_manager.py#L3655](../db_manager.py#L3655)
...
**Location**: [clashcontrol/db_manager.py](../db_manager.py#L962)
...
**Location**: [clashcontrol/db_manager.py](../db_manager.py#L1144) / [../db_manager.py#L1446](../db_manager.py#L1446)
...
**Location**: [clashcontrol/constants.py](../constants.py#L206)
...
before `normalize_cwl_season()` existed — deleted after use, no longer in `clashcontrol/scripts/`.
...
**Location**: [ClashControl.py](../../ClashControl.py#L237), inside `startup_login()`.

The `coc.Client(key_count=10, throttler=coc.BatchThrottler, throttle_limit=100)` construction
itself is documented in `../clashcontrol/docs/RATE_LIMITING_IMPLEMENTATION.md` (canonical source —
...
the database at all. `on_ready()` calls `ClashControl.initialize_database()` as its own "Step 1.5",
...
**Location**: [clashcontrol/cache_manager.py](../cache_manager.py#L176)

`self.coc_clan_cache = CoCClanCache(soft_ttl_seconds=280, hard_ttl_seconds=600)`

## Performance Investigation (2026-07-11)

**Tooling**: [clashcontrol/scripts/log_time_gaps.py](../scripts/log_time_gaps.py) is functional but
...
1. **Nightly full VACUUM took 2688s (44.8 min)** on the 33 GB `data/clashcontrol.db` (freed 2.4 GB).
   ...
   at 03:00 UTC (`ClashControl.py` ~line 2060, the `hour == 3, minute == 0` scheduling gate). Blocks DB-backed Discord commands via
...
3. **File-count scale** (2026-07-11 measurement): `data/temp/` = 30,522 files across 10 shards;
   ...
   (`clashcontrol/move_old_archive_files.py`, `days=7.0`, called nightly via `_archive_move_nightly()`
   in ClashControl.py before DB maintenance) — the large count reflects genuine scale (thousands of
...
4. Pre-scan optimization already exists (`ClashControl.py` ~line 952: single `glob.glob()` for temp
...
### Complete function inventory (verified against clashcontrol/db_manager.py; ALL items now DONE)
...
`clashcontrol/QBdiscocmdshelper_admin_command.py`) had the same raw-sqlite3/main-only gap — fixed.
...
- `WarHistoryDB.run_history_migration()` — orchestrates the nightly rolling migration. Every
  ...
  `CONFIG.history_migration_time_budget_minutes` bounds the automatic (ClashControl.py-triggered) paths;
  `run_history_migration_now.py --time-budget-minutes N` bounds a manual CLI run.
- `clashcontrol/scripts/run_db_maintenance_now.py` — CLI wrapper to trigger only
  ...
  **Correction (2026-08-01): this does NOT run the monthly migration** — `nightly_db_maintenance()`
  has no migration step; only `ClashControl.run_nightly_maintenance_routine()` (Step 0.5, day==1-gated)
  ...
  `clashcontrol/scripts/run_history_migration_now.py` (previously deleted, per the note this replaces)
...
- `attach_history_db(conn, db_path, history_db_path=None, read_only=False)` (module-level in
  `clashcontrol/db_manager.py`) — convenience helper for standalone scripts using a bare `sqlite3`
