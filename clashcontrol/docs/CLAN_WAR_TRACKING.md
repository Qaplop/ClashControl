...
| `track_war_updates` | clashcontrol/coc_cache.py | `_update_clan_metadata()` | War league change detected, clan not subscribed | Current value False and league entered Master III+ | True (promotion) |
| `track_war_updates` | clashcontrol/coc_cache.py | `_update_clan_metadata()` | War league change detected, clan not subscribed | Current value True and league dropped below Master III | False (demotion); also purges the clan's temp war files |
...
| `has_active_subscriptions` | clashcontrol/ui_clan_management.py | `MemberClansConfigurationView._on_apply()` | Guild member_clan added, clan not yet cached | Unconditional (both fetch-success and fetch-failure branches) | False |
| `has_active_subscriptions` | clashcontrol/cache_manager.py | `_sync_group_track_war_updates()` | Group league_rank freshly resolved (`_process_league_group_response`) | New group member not yet in `clan_name_cache` | False (inserted) |
| `track_war_updates` | clashcontrol/cache_manager.py | `_sync_group_track_war_updates()` | Group league_rank freshly resolved (`_process_league_group_response`) | Any non-subscribed member (new insert or existing, promotion or demotion) | Set to match the group's confirmed league (True for M3+, False otherwise) |
| `track_war_updates` | QBhelperfunctions.py | `refresh_stale_passive_clans()` | Monthly passive-clan sweep (Phase 1.6, ClashControl.py) | Passively-tracked clan's `get_clan()` ping reveals Master III+ | True (promotion, via the normal `_update_clan_metadata()` path this triggers) |

---

### 7. CWL group league confirmed (background, subscription-independent)

File: clashcontrol/cache_manager.py `_sync_group_track_war_updates()`, called from `_process_league_group_response()`
...
- **Mid-season guard (added 2026-08-08):** a demotion is deferred (only `war_league` corrected) if
  ...
  one-time backfill for this write-path (`clashcontrol/scripts/backfill_group_track_war_updates.py`, applied
  to prod 2026-08-08) derived `track_war_updates` from the currently in-progress season and demoted
  clans mid-way through it; remediated via `clashcontrol/scripts/repromote_mid_season_clans.py`. See path 5
...
- **Self-heal (added 2026-08-09):** the guard above only fires *at the moment of a demotion
  ...
  `war_summary` rows later the same season via `CWL-GROUP-EXPAND` (`ClashControl.py`), which force-fetches
...
File: QBhelperfunctions.py `refresh_stale_passive_clans()`, called from ClashControl.py Phase 1.6.
Candidate discovery: ClashControl.py's main clan-categorization loop (Step 1, see below).
...
ClashControl.py's main clan-categorization loop (Step 1 of the update cycle) already visits every clan every
...
`last_checked_via_api` is missing or older than `clashcontrol.constants.PASSIVE_CLAN_REFRESH_INTERVAL_DAYS` —
...
trickling it out over a month; see trade-off note below), ClashControl.py's Phase 1.6 then calls
...
- clashcontrol/ui_clan_management.py — admin clan-management function
...
Files: QBdiscordcmds.py, clashcontrol/ui_clan_management.py → cache_manager.py
...
File: clashcontrol/coc_cache.py `CoCClanCache._update_clan_metadata()`
...
- Demotion: if the clan's current `track_war_updates` is True and the new league drops out of
  ...
  **Mid-season guard (added 2026-08-08):** before applying the demotion, checks
  ...
  `clashcontrol/scripts/repromote_mid_season_clans.py`.
...
File: clashcontrol/ui_clan_management.py `MemberClansConfigurationView._on_apply()`
...
via a separate path: `clashcontrol/coc_cache.py` `_update_clan_metadata()` actively demotes non-subscribed
clans that drop below Master III (see write path 5 and Critical Constraint 1 above).

The league set is a single source of truth: `clashcontrol/constants.py` `WAR_UPDATE_LEAGUES`.
`clashcontrol/coc_cache.py`, `clashcontrol/cache_manager.py`, and `QBhelperfunctions.py`
...
File: clashcontrol/cache_manager.py `persist_clan()` → clashcontrol/db_manager.py `_save_clan_unlocked()`
...
| Location | Usage |
|---|---|
| ClashControl.py polling loop | Skip clans with `track_war_updates=False` entirely |
| ClashControl.py polling loop | Skip clans with `is_deleted=True` (after `track_war_updates` check) |
| ClashControl.py polling loop | True + has_active_subscriptions=True → polled every cycle (Tier 1) |
| ClashControl.py polling loop | True + has_active_subscriptions=False → polled only if >22h since last update (Tier 2) |
...
| clashcontrol/chart_clans_per_league.py | Stacked bar chart: solid = actively tracked, hatched = passively tracked |
...
1. track_war_updates is a one-way ratchet in `cache_manager.py` (`update_clan_subscription_status()`,
   ...
   **Exception (since 2026-06-29)**: `clashcontrol/coc_cache.py` `_update_clan_metadata()` DOES downgrade
...
2. Subscribed clans are doubly immune — if `has_active_subscriptions=True`, the upgrade signal
   ...
   skips such clans entirely, in both directions (clashcontrol/coc_cache.py).
...
### Auto-restore in `_update_clan_metadata()` (clashcontrol/coc_cache.py)
...
### Phase-1 loop skip (ClashControl.py)
...
| `is_deleted` | `clashcontrol/coc_cache.py` `_update_clan_metadata()` | Any successful `get_clan` response, clan was previously deleted | False |
| `is_deleted` | `clashcontrol/coc_cache.py` `_update_clan_metadata()` | New clan added for the first time | False |
...
|---|---|---|
| 2026-06-29 | Added CWL league demotion support — non-subscribed clans dropping below Master III now have `track_war_updates` reset to False (temp war files cleaned up); `_update_clan_metadata` is no longer a pure one-way ratchet | clashcontrol/coc_cache.py |
| 2026-05-31 | Added `is_deleted` clan deletion detection system | QBhelperfunctions.py, clashcontrol/coc_cache.py, clashcontrol/cache_manager.py, clashcontrol/db_manager.py, ClashControl.py |
| 2026-04-11 | Master II → Master III (lowered threshold) | _WAR_UPDATE_LEAGUES (×2), _CWL_HARVEST_LEAGUES |
| 2026-04-11 | track_war_updates made a one-way ratchet (never downgrades) | cache_manager.py, clashcontrol/coc_cache.py |
