...
- **CWL Clan-Config Discord Activity**: a Cloudflare Pages/Workers web app (`activity/`),
  ...
  `aiohttp.web` bridge (`clashcontrol/web_bridge.py`) that reuses the bot's own `CACHE`/`db_manager` —
...
- User-facing DB writes (`save_user`, `save_guild_config`, `save_subscriptions_for_channel`,
  ...
  (see ../clashcontrol/docs/DATABASE_ARCHITECTURE.md § Retry-on-Locked)
...
```
Every modal in `clashcontrol/` follows this since 2026-09-23 (registration, clan management, buddy
...
`clashcontrol/ui_tracker.py`'s `TrackerItemModal` (migrated 2026-08-29, tracker #0009 follow-up) and,
...
### Adding a new `clashcontrol/translations/{lang}.json` file
`i18n.py`'s `TranslationManager.load_translations()` auto-discovers every `*.json` in
`clashcontrol/translations/` by filename stem, so dropping in a new file (matching `en.json`'s exact
...
fires. Real-world example: `clashcontrol/ui_tracker.py`'s `TrackerItemModal.environment_select`.
...
below; the same persistence rule applies. Real-world example: `clashcontrol/ui_tracker.py`'s
...
```
For new code use the module-agnostic helpers in `clashcontrol/ui_common.py` (2026-09-23) rather than
reinventing the flag — `ui_tracker.py`'s older `_consume_once(view, interaction)` implements the
same shape for the tracker views:
```python
from clashcontrol.ui_common import claim_action, lock_buttons, release_action, unlock_buttons
...
🟧 clashcontrol/coc_cache.py (~710 lines)
...
🟧 clashcontrol/cache_manager.py (~3230 lines)
...
🟨 clashcontrol/db_manager.py (Database Layer - ~6200 lines)
...
- Chart data methods (2026-05-17+):
  ...
  · `get_global_db_statistics_sync(force_refresh=False)`: includes `players_tracked_count` (COUNT(*) FROM player_name_index).
    ...
    end of `run_nightly_maintenance_routine()` in ClashControl.py (after REINDEX/VACUUM/ANALYZE), so `/status` should never pay
    the cold-scan cost during normal operation. See `ClashControl._warm_global_db_stats_cache()`.
...
- Imports helpers from clashcontrol/QBdiscocmdshelper.py
...
🟩 clashcontrol/ui_common.py (~220 lines)
...
🟩 clashcontrol/ui_registration.py (~1920 lines)
...
🟩 clashcontrol/ui_notifications.py (~1430 lines)
...
🟩 clashcontrol/ui_clan_management.py (~6550 lines)
...
🟩 clashcontrol/guild_role_manager.py
...
🟫 clashcontrol/QBdiscocmdshelper.py (~4713 lines)
...
🟫 clashcontrol/exceptions.py
- Custom exception hierarchy (21 exception classes)
- Base: ClashControlError with message and context support
...
🟫 clashcontrol/discord_health.py
...
- `bulk_sync_global_commands()`: raw-HTTP global command bulk-upsert that always re-includes
  ...
  outright, HTTP 400/50240, instead of deleting it). Used in `ClashControl.py` in place of
...
🟫 clashcontrol/web_bridge.py
- `aiohttp.web` app for the CWL Clan-Config Discord Activity (CWL_CLAN_CONFIG_ACTIVITY_PLAN.md
  Phase B), started from `ClashControl.py`'s `_setup_hook()` / stopped from `async_cleanup()` —
...
- `/api/tracker/*` (BUG_FEATURE_TRACKER_PLAN.md Phase 6): list/get items, download attachment
  ...
  DB+Discord work to `clashcontrol/ui_tracker.py` (`apply_status_change()`, `post_comment()`,
  `post_test_cases()`) rather than duplicating it.

🟩 clashcontrol/ui_tracker.py (BUG_FEATURE_TRACKER_PLAN.md, Phases 2-5)
- Discord UI layer for the bug/feature tracker — kept separate from `ui_clan_management.py`
  ...
  from `ClashControl.py`'s `on_message`); `apply_status_change()`/`TrackerStatusSelectView` (status
  ...
  from `ClashControl.py`'s `on_raw_reaction_add`). CACHE.tracker_settings caches `bot_settings`
...
🟫 clashcontrol/mcp/ (BUG_FEATURE_TRACKER_PLAN.md Phase 7)
- `tracker_mcp.py`: stdio MCP server (`python -m clashcontrol.mcp.tracker_mcp`), hand-rolled
...
### Changes Made
1. **Created clashcontrol/ui_components.py** (~260 lines)
  - Note: This was later split into `clashcontrol/ui_common.py`, `clashcontrol/ui_registration.py`, `clashcontrol/ui_notifications.py`, `clashcontrol/ui_clan_management.py` and `clashcontrol/ui_components.py` was deleted (2026-02-17)
  - Note: `clashcontrol/ui_welcome.py` was renamed to `clashcontrol/ui_registration.py` and `WelcomeView` → `RegistrationView` as part of the registration-message refactor
  - Originally moved UI classes from QBdiscordcmds.py (e.g. WelcomeView, PlayerSubstringModal)
   
2. **Enhanced clashcontrol/QBdiscocmdshelper.py** (+400 lines)
...
### 🟦 Regular Update Cycle (ClashControl.py) - Multi-Phase Pipeline
...
└── main()
    ...
    │       HDD/server-machine are not re-read every cycle (minimizes server-machine I/O).
        ...
        │   │   ├── _merge_entries()
        │   │   │   └── uses MODE_REGISTRY from clashcontrol/formatting.py 🟫
...
└── Discord command handlers
    ...
    │   │   automatic posting loop in ClashControl.py so the rendered period always matches
    ...
    │       period, matching ClashControl.py's loop, so the fallback still updates the latest
...
🟦 ClashControl.py — full call graph in Main Flows § Regular Update Cycle; inventory:
...
│   ├── calculate_leaderboard()
│   │   ├── _merge_entries() — player name change handling via PlayerID; uses MODE_REGISTRY (clashcontrol/formatting.py 🟫)
...
│                                    # clashcontrol/QBdiscocmdshelper_admin_command.py's
...
├── handle_tracker_upload_message()         # called from ClashControl.py's on_message
├── apply_status_change() / TrackerStatusSelectView
├── post_test_cases() / TrackerTestPassButton / TrackerTestFailButton
└── handle_tracker_test_reaction()          # called from ClashControl.py's on_raw_reaction_add
