...
See also: ../clashcontrol/docs/CODE_STRUCTURE.md § Select Dropdown Persistence
...
# Yes/No confirm dialog and a status-select dropdown in clashcontrol/ui_tracker.py both did exactly
...
See also: ../clashcontrol/docs/CODE_STRUCTURE.md § Discord.py Patterns
...
See also: ../clashcontrol/docs/CODE_STRUCTURE.md § Discord.py Patterns (Single-Message Flow, Double-Click Guard)
...
See also: ../clashcontrol/docs/CODE_STRUCTURE.md § Cache Management
...
One-off scripts under `clashcontrol/scripts/` may use direct sqlite3 for offline validation/repair.

See also: ../clashcontrol/docs/DATABASE_ARCHITECTURE.md
...
Single-scan pattern in `load_all_temp_war_stats()` (clashcontrol/cache_manager.py):
...
Code defences also in place in ClashControl.py (independent of the M×N glob fix):
...
See also: ClashControl.py `periodic_main()` exit-code block

See also: ../clashcontrol/docs/DATABASE_ARCHITECTURE.md § Migration Principles (Principle 4)
...
Root cause: The Phase-1 build loop in `ClashControl.py` has TWO separate early-continue guards:
...
automatically by `_update_clan_metadata()` in clashcontrol/coc_cache.py on any successful
`GET /clans/{tag}`. Do NOT add `is_deleted` to `_ESSENTIAL_CLAN_FIELDS`.

See also: ../clashcontrol/docs/CLAN_WAR_TRACKING.md § Clan Deletion Detection
...
`handle_cleanup_messages_all()` in clashcontrol/QBdiscocmdshelper_admin_command.py (fixed 2026-08-03).
...
risk shows up without `get_channel()` at all. `repost_playerregistration_messages()` in ClashControl.py
...
See also: clashcontrol/QBdiscocmdshelper_admin_command.py `handle_cleanup_messages_all()`,
ClashControl.py `repost_playerregistration_messages()` / `cleanup_stale_ui_messages()`
...
loop in `main()` (ClashControl.py) iterating `CACHE.clan_name_cache` (~380K entries in PROD) cost
...
One-off cleanup script for existing bad PROD rows: `clashcontrol/scripts/cleanup_orphaned_clans.py`
...
`PlayerSubstringModal.on_submit()` (clashcontrol/ui_registration.py). Check for this same ordering bug
...
1. **View-level gate**: any `discord.ui.View` registered via `bot.add_view()` must override
   ...
   `RegistrationView.interaction_check()` (clashcontrol/ui_registration.py). NOT automatic for future
   `add_view()`-registered views (e.g. planned CWL hubs) — each needs the override.
2. **Cache-level gate** (`CacheManager.users_loaded`, clashcontrol/cache_manager.py): `False` until
...
once when it ends does. Running `clashcontrol/scripts/log_time_gaps.py --top 400` over the full day
...
above `_post_cycle_cleanup()` in ClashControl.py, which already flagged this as an accepted gap: "Python's
...
Fix (both applied near the top of ClashControl.py / in the Step-3 cache-load block):
...
Diagnostic tool: same as Pitfall 16 — `clashcontrol/scripts/log_time_gaps.py --top 300-400`, looking for
...
maintenance_routine()` (`ClashControl.py`) now ends with `gc.unfreeze()` + a real full `gc.collect()`
...
## Pitfall 22: `ClashControl.py`'s top-level code runs TWICE in one process — `QBdiscordcmds.py` imports back from it
...
the `ClashControl.py` → `QBdiscordcmds.py` → `ClashControl.py` cycle — a real fix, but a bigger one that needs
its own review of every consumer.

Rule going forward: any new module-level code added near the top of `ClashControl.py` that does more than
...
`^[A-Za-z_].*\.(append|add|register)\(` at column 0 in `ClashControl.py` to audit for more of these.
...
(already done by `normalize_player_name()` in `clashcontrol/formatting.py`: `f"{LRM}{name}{LRM}"`), AND
...
```
No logging (`coc.NotFound` wasn't logged at any level — see `coc_retry()` in `clashcontrol/coc_health.py`),
no retry, no indication in the output that data was incomplete. DEV and PROD log in via *separate*
Clash of Clans developer accounts (`COC_API_EMAIL_DEV` vs `COC_API_EMAIL`, see `clashcontrol/config.py`).
...
(`clashcontrol/coc_health.py`, scoped to `operation_name.startswith("get_league_war")` so it doesn't add noise
...
(`coc.py`'s default — nothing in this repo overrides it, so every script under `clashcontrol/scripts/` that
...
- **Proactive detection at startup**: new `_validate_coc_api_keys()` (`ClashControl.py`), fired as a
...
- **Temporary diagnostics reverted**: the verbose `[COC-PRIVATEWARLOG-DIAG]` block in `coc_retry()`
  (`clashcontrol/coc_health.py`) and the DEV-only `coc.http` logger `INFO` override (`ClashControl.py`) are both
...
(`clashcontrol/discord_health.py`) instead — it fetches the app's current global commands via a raw
...
Root cause: `clashcontrol/ui_cwl_roster.py`'s `_make_cwl_management_add_season_callback` called
...
1. **Every `db.*_sync()` call with a write verb (`create_`, `set_`, `update_`, `delete_`,
   ...
   elsewhere (e.g. `ClashControl.py`'s startup `check_hot_history_schema_parity_sync()` call).
...
`clashcontrol/scripts/log_time_gaps.py` (see Pitfall 16) against the live PROD log around the
...
**Full-sweep follow-up, same day**: an audit of every `clashcontrol/ui_*.py`, `QBdiscocmdshelper*.py`,
and `clashcontrol/web_bridge.py` write path found the anti-pattern was not a one-off — the entire CWL
...
Root cause: `CACHE` is imported once at module level (`from clashcontrol.cache_manager import CACHE`,
near the top of the file) — that should make it available everywhere. But three of `admin()`'s
individual action branches ALSO had their own `from clashcontrol.cache_manager import CACHE` line,
...
Symptom (2026-08-17, PROD): `ClashControl.py`'s startup DB initialization is wrapped in
...
`POST /api/cwl/enrollment/guest-players/remove`) were added to `clashcontrol/web_bridge.py` — routes
...
`clashcontrol/web_bridge.py` (the actual Python bridge) and does two jobs neither side can do alone:
...
Fix pattern used here (`clashcontrol/ui_tracker.py`, `start_tracker_item()`): call
...
**Symptom (2026-08-22, live report):** the tracker MCP server (`clashcontrol/mcp/tracker_mcp.py`,
...
**Why:** `clashcontrol/ui_tracker.py`'s bug/feature tracker is meant to be PROD-only —
...
sign-off shortcut is wired as `on_raw_reaction_add` in `ClashControl.py`, registered unconditionally
...
(`clashcontrol/mcp/tracker_mcp.py`).
...
`clashcontrol/ui_common.py`'s `claim_action()` / `release_action()` / `lock_buttons()` /
...
**Why:** `_build_testcases_moved_message()` (`clashcontrol/ui_tracker.py`) returns `view=None`
...
repost call (`ClashControl.py`'s `main()`, `only_if_not_bottom=True`), which genuinely does skip a
...
in `CwlManagementHubView`'s own docstring (`clashcontrol/ui_cwl_roster.py`) so a future reader doesn't
...
**Root cause:** `repost_anchored_message()` (`ClashControl.py`) and its three thin wrappers
...
**Root cause:** `_make_cwl_settings_toggle_callback()` (`clashcontrol/ui_cwl_roster.py`) toggled the
...
**Root cause:** `add_cwl_settings_components()` (`clashcontrol/ui_cwl_roster.py`) is a *shared content
...
`9.options.0.choices.17.name` = `COMMANDS[9]` in `ClashControl.py`'s `setup_hook()` list (`/admin`),
...
`clashcontrol.ui_common.check_maintenance_block` for components) deliberately do **not** consult the
...
2. **A memo cache that missed exactly when it mattered.** `_backdated_for()` in `ClashControl.py`
...
**What happened (2026-09-04):** `ClashControl.py` carried
...
values())` snapshot — the same idiom `ClashControl.py` already uses for the identical hazard. Tests
...
- **Why this was still worth writing down even though it never shipped:** this is exactly the
  ...
  like the established, trusted `ClashControl.py:1216` pattern), and would only have surfaced live in
...
(`clashcontrol/ui_tracker.py`, added for ticket #0102 two days earlier) was built with
`discord.ui.Label(text="Your reply (optional — leave blank to just grant/invite without a
message)", ...)` — 76 characters. `clashcontrol/docs/CODE_STRUCTURE.md` already documents "`Label.text`
...
(`clashcontrol/ui_cwl_roster.py`) had its own `async def _refresh(self, interaction)` to re-render
