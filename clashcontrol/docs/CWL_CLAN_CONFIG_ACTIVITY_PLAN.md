...
Cloudflare Tunnel (cloudflared, free)  ──  runs alongside the ClashControl process
   |
   v
ClashControl bridge API (new: clashcontrol/web_bridge.py, aiohttp.web)
...
## Bridge API (ClashControl side) — `clashcontrol/web_bridge.py`

New file, new `aiohttp.web.Application`, started from `ClashControl.py`'s `_setup_hook()` alongside the bot (`web.AppRunner` + `web.TCPSite` bound to `127.0.0.1:<port>`, never bound to `0.0.0.0` — only `cloudflared` should ever reach it). Exactly two endpoints for MVP scope:
...
What's still planned for Phase C: a **"Open Clan Config (Web)"** button next to "Configure Participating Clans" in `add_cwl_management_components()` (`clashcontrol/ui_cwl_roster.py`), for in-context launch from the screen an admin is already on, rather than requiring them to discover `/launch` separately. discord.py has no high-level wrapper for responding to a component interaction with a `LAUNCH_ACTIVITY` callback (type 12), so that button's callback will still need a raw REST call through `QBcore.bot.http` — this part is **not yet verified**, only the auto-created entry point command is. Worth a quick spike at the start of Phase C before assuming it works identically.

**Launches the bot didn't start → landing page (tracker #0135, 2026-09-25).** Once the app was listed in the App Directory, its profile's **Launch** button (the same auto-created entry point) became the most visible way in, used by people who have never seen a bot button. Such a launch has no `pending_cwl_activity_screen` entry, so `GET /api/cwl/screen` answers `landing` (it used to fall back to `clan_config`), and a launch outside any server with no `pending_cwl_dm_guild` entry goes straight to the landing page too (it used to stop at "must be launched from inside a guild"). `activity/client/src/landingPage.ts` renders it from the `activity.landing` translation namespace: what ClashControl does, an "Add to server" button (`openExternalLink` to `discord.com/oauth2/authorize?client_id=…`, so the Developer Portal's install settings decide scopes/permissions), first commands for members and admins, and a button to ClashControl's public support channel on The QCrew server (`SUPPORT_URL` in `landingPage.ts`, a permanent invite rather than a channel link, which only opens for existing members; the same invite is `SUPPORT_INVITE_URL` in `clashcontrol/constants.py` for `/about`'s text fallback). `GET /api/i18n` accepts a missing `guild_id` for this. The landing page also asks for `use_discord_locale=1`: the Worker then forwards the user's Discord client language (`locale` from `/users/@me`, already covered by the `identify` scope) as `discord_locale`, and the bridge's order becomes manually chosen bot language (`user_language_locked`) → Discord language (`i18n.language_from_discord_locale`, "es-419" → es, "zh-TW" → zh) → stored "auto" bot language → server language → English. An unlocked language is only the bot's own guess from an older interaction, so the current Discord language outranks it (live PROD finding 2026-09-25: a stored "en" kept the page English for German and Spanish Discord users). Each landing load logs one `[I18N] activity.landing for user …` line with every input and the result. Other screens don't ask for it and keep matching the bot's DMs. Every bot button records its screen in `_launch_cwl_activity` before launching, so none of them can reach the landing default; the one trade-off is a pop-out re-run after a bot restart (the dicts are in memory only).

**Launch hints are claimed per Activity instance (tracker #0136).** Read non-destructively, a recorded screen / DM server lingered until the next click, so after `/cwl preferences` once, Discord's Launch button kept reopening that screen. The client now sends `discordSdk.instanceId` with `/api/cwl/screen` and `/api/cwl/dm-guild`; the bridge's `_claim_launch_hint` pops the recorded value on first read and remembers it in `CACHE.cwl_activity_instance_claims` for that instance (pop-out reloads of the same instance keep it; any new launch finds nothing → landing page). A newly recorded value always wins over an older claim, for clicking another CWL button while the Activity is open. Requests without `instance_id` (older client) keep the non-destructive read, so bridge and Activity deploy in either order. **`/about`** opens the landing page on purpose: it records `landing` in a server, or drops the recorded DM server in the DM, and launches; a refused launch (e.g. unverified app in DMs, 50106) falls back to `commands.about.fallback` (install link + first commands).
...
(`clashcontrol/discord_health.py`): fetches the app's current global commands via a raw HTTP call,
filters for an existing Entry Point command, and always splices it back into the bulk-upsert
payload — used at both call sites in `ClashControl.py` in place of `tree.sync(guild=None)`. This is a
...
- Any change to ClashControl's core `db_manager.py`/`CACHE` architecture — the bridge API is purely a new *read/write client* of the existing layer, per Cardinal Rule 3 (CACHE-only data access).
