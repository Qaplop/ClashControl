...
(`GET /api/cwl/guest-search`, backend `_search_cwl_guests()` in `clashcontrol/web_bridge.py`):
...
4. `CACHE.search_player_names()` (`clashcontrol/cache_manager.py`) scans all 6.6M index entries calling
...
- Read `.github/copilot-instructions.md` in full first. Rules that matter most here:
  ...
  - **Pitfall 26** (`clashcontrol/docs/COPILOT_PITFALLS_COOKBOOK.md`): sync SQLite calls in async
    context must be bundled into `asyncio.to_thread()` hops — one hop per atomic unit of work.
    Follow the exact pattern already used by the write handlers in `clashcontrol/web_bridge.py`
...
- **Docs (Rule 15)**: update `clashcontrol/docs/CWL_CLAN_CONFIG_ACTIVITY_PLAN.md` (and
...
**File**: `clashcontrol/web_bridge.py`, function `_search_cwl_guests()` (~line 527).
...
**File**: `clashcontrol/web_bridge.py`, `_search_cwl_guests()` clan loop (~line 636).
...
**File**: `clashcontrol/web_bridge.py`, `_search_cwl_guests()` and its caller
...
**File**: `clashcontrol/db_manager.py`, `get_player_links_sync()` (~line 3710).
...
**File**: `clashcontrol/web_bridge.py`.
...
**Docs**: extend Pitfall 26 in `clashcontrol/docs/COPILOT_PITFALLS_COOKBOOK.md` with one sentence: the
...
**Files**: `clashcontrol/web_bridge.py` (preferred cache location: bridge-side, since the Discord-side
...
hookable (there is an existing per-cycle cleanup — `[CYCLE-CLEANUP]` in `ClashControl.py` — clear it
...
always-fresh-vs-cached split was needed. `clear_player_stats_cache()` wired into `ClashControl.py`'s
...
**Files**: `clashcontrol/web_bridge.py`, `activity/server/src/index.ts` (one new proxy route),
...
1. **Backend version counter**: module-level `_enrollment_version: Dict[str, int]`
   ...
   `clashcontrol/QBdiscocmdshelper_cwl.py`/`clashcontrol/ui_cwl_roster.py` that touch cwl_signups /
...
**File**: `clashcontrol/cache_manager.py`, `search_player_names()` (~line 434) and the
...
1. **Stop lowercasing 6.6M names per call**: maintain lowercase forms built once at load time.
   ...
   - `clashcontrol/cache_manager.py:234` — declaration `self.player_name_index: Dict[str, str]`
     (type annotation changes to `Dict[str, Tuple[str, str]]`).
   - `clashcontrol/cache_manager.py:424` — bulk load via
   ...
   - `clashcontrol/cache_manager.py:432` — error-fallback reset `= {}` (no change needed).
   - `clashcontrol/cache_manager.py:449` — reader in `search_player_names()`.
   - `clashcontrol/coc_cache.py:678` — incremental writer in `update_player_info_in_user_accounts()`.
   - `clashcontrol/web_bridge.py:664` — reader in `_search_cwl_guests()` `#` tag mode (values unused
...
**File**: `ClashControl.py` (the existing `[GC-AUTO]` logger + startup `gc.freeze()` block, ~line 121,
...
2. Nightly re-freeze — **verified 2026-08-17: this does NOT exist yet and must be implemented**
   (the only `gc.freeze()` in the codebase is the startup one, ClashControl.py ~line 121; the nightly
   `run_nightly_maintenance_routine()` at ClashControl.py ~lines 2381–2462 never re-freezes). Add
   ...
   Update Pitfall 21 in `clashcontrol/docs/COPILOT_PITFALLS_COOKBOOK.md` in the same change: document
...
2026-08-17 by reading `ClashControl.py` directly (not just searching `COPILOT_PITFALLS_COOKBOOK.md`,
...
**Files**: `clashcontrol/db_manager.py` (new table + query helpers), `clashcontrol/cache_manager.py`
(`search_player_names` delegates; `player_name_index` load/retention decision), `ClashControl.py`
...
`clashcontrol/docs/DATABASE_ARCHITECTURE.md` ("2026-08-17: player_name_index Scan-Cost Fix + SQLite/
...
A full sweep of `.github/copilot-instructions.md`, `clashcontrol/docs/*.md`, `activity/README.md`,
...
- **`[CYCLE-CLEANUP]` hook** (Step 7's cache clear): ClashControl.py ~lines 2312–2330 in
