...
All CWL tables live in the **hot DB only** (`clashcontrol/db_manager.py`'s schema-creation block,
...
(`BotEmojis.CWL_BENCH`, generated to `clashcontrol/icons/emoji/cwl_bench.webp`) — part of the general
...
**i18n**: `clashcontrol/translations/{en,de}.json`, top-level `cwl.*` namespace — `settings`,
...
`WarHistoryDB.purge_expired_cwl_events()`, run as Step 0.6 of `ClashControl.py`'s nightly maintenance —
...
the top. It lived at the repo root until 2026-08-30 and moved to `clashcontrol/docs/` once the feature
...
`clashcontrol/docs/CODE_STRUCTURE.md`, not repeated here.
