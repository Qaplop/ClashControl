...
ratchet** (see `clashcontrol/docs/CLAN_WAR_TRACKING.md`): only clans that are
...
for their own perspective. `CWL-GROUP-EXPAND` (`ClashControl.py`) force-polls all 8
...
Practical consequence for `clashcontrol/scripts/analyze_cwl_rounds.py --fetch-missing`:
...
**2026-08-08 fix**: `_sync_group_track_war_updates()` (`clashcontrol/cache_manager.py`,
...
`CWL-GROUP-EXPAND` (`ClashControl.py`, ~line 1049) force-fetches all 8 members of any CWL group
...
**Fixed 2026-08-09**: `_sync_group_track_war_updates()` (`clashcontrol/cache_manager.py`) now also
...
- [x] Remove the `_ws_migrations` list and `_legacy_indexes` DROP list from `db_manager.py` — confirmed removed; no references remain in `clashcontrol/db_manager.py`
