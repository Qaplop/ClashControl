...
- **`_FETCH_CONCURRENCY`** (`ClashControl.py`): raising it adds latency and in-flight memory,
...
- **`throttle_limit` / the throttler**: the configured 100 req/s was never reached;
  ...
  `ClashControl.py` fixes that. But fixing it changed no cycle time, because the gate was
...
`CoCClanCache` (`clashcontrol/coc_cache.py`) caches `coc.Clan` objects for 280 s soft / 600 s hard,
