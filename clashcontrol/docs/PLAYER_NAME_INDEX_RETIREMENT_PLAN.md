...
`clashcontrol/docs/DATABASE_ARCHITECTURE.md`'s player_name_fts section, Pitfalls 16/29 in the
...
1. **Line-number drift**: those fixes reshaped `clashcontrol/db_manager.py` twice — every citation into
...
- Read `.github/copilot-instructions.md` in full first, particularly: Rule 11 (DB access via
  ...
  Pitfall 26 (`clashcontrol/docs/COPILOT_PITFALLS_COOKBOOK.md`) — sync SQLite calls reached from the
...
- **Docs**: update `clashcontrol/docs/DATABASE_ARCHITECTURE.md` (it already documents
  `player_name_index`/`player_name_search`/`player_name_fts` under its own "2026-08-17" section —
  extend that section, don't create a new one) and `clashcontrol/docs/CODE_STRUCTURE.md` (it lists
...
**File**: `clashcontrol/db_manager.py`, next to `search_player_names_sync` (~line 5389 as of commit
...
`clashcontrol/docs/COPILOT_PITFALLS_COOKBOOK.md` generally on not duplicating subtle logic).
...
(new key, both `en` and `de` locale files — check `clashcontrol/locales/` or wherever `t()`'s
...
`clashcontrol/docs/DATABASE_ARCHITECTURE.md` for the full writeup). 2061 tests pass (net -10 vs Batch
...
- `clashcontrol/coc_cache.py`: the `cache_manager.set_player_name(tag, name)` call (~line 678) inside
...
- `clashcontrol/web_bridge.py` (~line 772-786, `_search_cwl_guests_sync`'s tag-prefix mode): the
...
**File**: `clashcontrol/db_manager.py` (~line 5457 as of commit `b02b73a`; line numbers in this file
...
(`clashcontrol/docs/CODE_STRUCTURE.md:328` already calls it "superseded by in-memory index at runtime").
