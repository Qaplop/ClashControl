...
`clashcontrol/docs/COPILOT_PITFALLS_COOKBOOK.md` Pitfall 24 for the incident (2026-08-09) this guards against —
...
`ClashControl.py: _validate_coc_api_keys()` runs as a fire-and-forget background task right after
...
processing). The actual phases emitted by `ClashControl.py` are tagged `[PHASE-1]`,
...
Everything above bounds *outbound CoC API* calls. `clashcontrol/guild_role_manager.py` has a separate,
...
### 1. Compatibility shims — `apply_coc_library_patches()` (`clashcontrol/coc_health.py`)
...
### Strategy: cycle-wide fast-fail (`clashcontrol/coc_health.py`)
...
- After `asyncio.gather()` completes Phase 1, `ClashControl.main()` checks `is_maintenance_detected()`
