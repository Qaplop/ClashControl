...
```python
import pytest
from clashcontrol.formatting import (
...
| **CONFIG singleton** | `patch("clashcontrol.config.CONFIG", ...)` with test values | No `.env` file needed |
...
def mock_config():
    ...
    with patch("clashcontrol.config.CONFIG", fake_config):
        yield fake_config
...
| `ClashControl.py` main loop orchestration | Better covered by E2E; mocking everything is fragile |
