...
`python -m clashcontrol.mcp.tracker_mcp`.

Env vars (see `.mcp.json` / `.vscode/mcp.json`, falling back to `.env` — loaded below, same
convention as `clashcontrol/config.py` — since the CLI host's `${env:VAR}` substitution isn't
...
from clashcontrol.mcp.tracker_bridge_client import TrackerBridgeClient, TrackerBridgeError
...
SERVER_NAME = "clashcontrol-tracker"
