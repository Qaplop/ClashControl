...
Background (see changelog.txt 2026-08-08 (2)/(4), clashcontrol/docs/CLAN_WAR_TRACKING.md
write-path 7, and clashcontrol/docs/CWL_ROUND_TRACKING_PLAN.md "Why war_summary coverage
is inherently asymmetric"):

    `_sync_group_track_war_updates()` (clashcontrol/cache_manager.py) corrects a CWL
    ...
    safety fix, see clashcontrol/scripts/audit_cwl_league_rank.py). Every group already
    ...
    `_sync_group_track_war_updates()`'s logic:
      ...
      - Every other clan gets `track_war_updates` set to whether its most
        recent league is Master III+ (`clashcontrol.constants.WAR_UPDATE_LEAGUES`),
    ...
    `clashcontrol.constants.normalize_cwl_season`).
...
Usage:
    python -m clashcontrol.scripts.backfill_group_track_war_updates
    python -m clashcontrol.scripts.backfill_group_track_war_updates --limit 500
    python -m clashcontrol.scripts.backfill_group_track_war_updates --apply
...
from clashcontrol.constants import WAR_UPDATE_LEAGUES
from clashcontrol.db_manager import attach_history_db
...
def find_corrections(conn: sqlite3.Connection) -> List[_Correction]:
    ...
    clashcontrol/scripts/repromote_mid_season_clans.py). Deferred clans self-correct
