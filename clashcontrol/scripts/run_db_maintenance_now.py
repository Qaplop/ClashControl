...
night (see ClashControl.py's maintenance scheduler). This script lets an operator run
...
`ClashControl.run_nightly_maintenance_routine()` wrapper — so it skips that wrapper's Step 0.5
...
def _configure_logging(log_file: str | None) -> None:
    ...
    silently dropped. Same format ClashControl.py's own logging uses.
