...
`run_nightly_maintenance_routine()`, gated by `ClashControl.py`'s `is_history_migration_due()`
...
def _configure_logging(log_file: str | None) -> None:
    ...
    with zero output for however long it takes. Same format ClashControl.py's own
    logging uses.
