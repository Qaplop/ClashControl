...
ClashControl.py wires the two module-level hooks this file exposes for events it doesn't own:
...
from clashcontrol.i18n import t as _t_localized
from clashcontrol.ui_common import claim_action, lock_buttons, unlock_buttons


def t(key: str, **kwargs: Any) -> str:
    """Shadows clashcontrol.i18n.t for this whole module: the bug/feature tracker RECORD (the item
    ...
    individually -- clashcontrol.i18n.t() already defaults to English whenever neither is given.
...
async def handle_tracker_upload_message(message: discord.Message) -> bool:
    """Called from ClashControl.py's on_message handler, BEFORE the DM-fallback branch (plan §5.4) —
...
async def apply_pending_requestor_access(member: discord.Member) -> None:
    """Called from ClashControl.py's on_member_join: finishes what _handle_grant_access()'s
...
class TrackerReplyModal(discord.ui.Modal, title="Reply to requestor"):
    ...
    # clashcontrol/docs/CODE_STRUCTURE.md's "Text Input Labels" section) -- the original text here was
...
async def _edit_to_testcases_moved_message(
    ...
    clashcontrol/docs/COPILOT_PITFALLS_COOKBOOK.md)."""
    text, view = _build_testcases_moved_message(item_number, result, guild_id)
...
async def handle_tracker_test_reaction(payload: discord.RawReactionActionEvent) -> None:
    ...
    which Discord routes only to the owning application. ClashControl.py's own `on_raw_reaction_add`
    ...
    `not is_dev_mode` (clashcontrol/config.py) — this is the one function actually reachable from that
