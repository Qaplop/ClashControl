...
async def bump_enrollment_version(guild_id: Optional[int] = None) -> None:
    ...
    call site in this module and in clashcontrol/ui_cwl_roster.py is exactly the right hook point (they
...
def _build_clan_config_payload_sync(guild_id: int) -> Dict[str, Any]:
    ...
    from clashcontrol.QBdiscocmdshelper_cwl import cwl_league_rank, resolve_guild_member_clan_tags, resolve_selected_cwl_season
...
def _build_enrollment_payload_sync(guild_id: int) -> Dict[str, Any]:
    ...
    from clashcontrol.emojis import th_icon_url
...
def _search_cwl_guests_sync(guild_id: int, query: str) -> List[Dict[str, Any]]:
    ...
    from clashcontrol.QBdiscocmdshelper_cwl import resolve_cwl_pool_clan_tags_sync, resolve_selected_cwl_season
...
async def handle_get_i18n(request: web.Request) -> web.Response:
    ...
    from clashcontrol.i18n import get_guild_language, get_namespace, get_user_language, language_from_discord_locale
...
def _resolve_player_prefs_season_sync(
    ...
    from clashcontrol.QBdiscocmdshelper_cwl import get_current_cwl_event_sync
...
async def handle_post_cwl_player_prefs_status(request: web.Request) -> web.Response:
    ...
    from clashcontrol.ui_cwl_roster import _apply_cwl_signup_response, rerender_cwl_dm_after_response  # pyright: ignore[reportPrivateUsage]  # deliberately shared, see comment above
...
def clear_player_stats_cache() -> None:
    """Wholesale invalidation, called once per update cycle (ClashControl.py's [CYCLE-CLEANUP] hook) so
...
async def handle_post_cwl_enrollment_assign(request: web.Request) -> web.Response:
    ...
    from clashcontrol.QBdiscocmdshelper_cwl import assign_cwl_player_sync, resolve_selected_cwl_season
    from clashcontrol.ui_cwl_roster import refresh_cwl_management_hub_message
...
async def handle_post_cwl_enrollment_guest(request: web.Request) -> web.Response:
    ...
    from clashcontrol.QBdiscocmdshelper_cwl import resolve_selected_cwl_season, get_cwl_guest_clan_tags_sync
    from clashcontrol.ui_cwl_roster import refresh_cwl_management_hub_message
    ...
    def _check_and_upsert_sync() -> Optional[str]:
        ...
        from clashcontrol.QBdiscocmdshelper_cwl import _seed_status_from_global_sync  # pyright: ignore[reportPrivateUsage]  # deliberately shared with start_cwl_enrollment's own seeding, see comment above
...
async def handle_post_clan_config(request: web.Request) -> web.Response:
    ...
    from clashcontrol.ui_cwl_roster import notify_cwl_clan_shared, refresh_cwl_management_hub_message
...
async def handle_post_cwl_shared_clan_evict(request: web.Request) -> web.Response:
    ...
    from clashcontrol.QBdiscocmdshelper_cwl import evict_guild_from_shared_clan, resolve_selected_cwl_season
    from clashcontrol.ui_cwl_roster import refresh_cwl_management_hub_message
...
async def handle_post_cwl_activity_closed(request: web.Request) -> web.Response:
    ...
    ClashControl's own code never sets directly (no `disabled=True` anywhere tied to Activity session
...
async def handle_post_cwl_send_roster_updates(request: web.Request) -> web.Response:
    ...
    from clashcontrol.QBdiscocmdshelper_cwl import resolve_selected_cwl_season, send_cwl_roster_updates
...
async def _dm_pending_roster_updates_notice(guild_id: int, discord_user_id: int) -> None:
    ...
    from clashcontrol.ui_cwl_roster import CwlPendingUpdatesDmView
...
async def handle_post_tracker_create_item(request: web.Request) -> web.Response:
    if not _check_secret(request):
        return web.json_response({"error": "forbidden"}, status=403)
    from clashcontrol.ui_tracker import create_tracker_item_for_agent
...
async def handle_post_tracker_status(request: web.Request) -> web.Response:
    if not _check_secret(request):
        return web.json_response({"error": "forbidden"}, status=403)
    from clashcontrol.ui_tracker import STATUS_VALUES, apply_status_change
...
async def handle_post_tracker_comment(request: web.Request) -> web.Response:
    if not _check_secret(request):
        return web.json_response({"error": "forbidden"}, status=403)
    from clashcontrol.ui_tracker import post_comment
...
async def handle_post_tracker_reply_and_invite(request: web.Request) -> web.Response:
    ...
    if not _check_secret(request):
        return web.json_response({"error": "forbidden"}, status=403)
    from clashcontrol.ui_tracker import reply_and_invite_for_agent
...
async def handle_get_tracker_thread(request: web.Request) -> web.Response:
    if not _check_secret(request):
        return web.json_response({"error": "forbidden"}, status=403)
    from clashcontrol.ui_tracker import get_thread_messages
...
async def handle_post_tracker_testcases(request: web.Request) -> web.Response:
    if not _check_secret(request):
        return web.json_response({"error": "forbidden"}, status=403)
    from clashcontrol.ui_tracker import post_test_cases
...
async def handle_post_tracker_testcase_pass(request: web.Request) -> web.Response:
    if not _check_secret(request):
        return web.json_response({"error": "forbidden"}, status=403)
    from clashcontrol.ui_tracker import mark_environment_passed_and_refresh
...
async def handle_post_tracker_testcase_fail(request: web.Request) -> web.Response:
    if not _check_secret(request):
        return web.json_response({"error": "forbidden"}, status=403)
    from clashcontrol.ui_tracker import mark_testing_failed
...
async def handle_post_tracker_testcase_result(request: web.Request) -> web.Response:
    ...
    if not _check_secret(request):
        return web.json_response({"error": "forbidden"}, status=403)
    from clashcontrol.ui_tracker import mark_testcase_by_id_and_refresh
...
async def handle_post_tracker_testcase_move_done(request: web.Request) -> web.Response:
    if not _check_secret(request):
        return web.json_response({"error": "forbidden"}, status=403)
    from clashcontrol.ui_tracker import finalize_testcases_move
...
async def start_web_bridge() -> None:
    ...
    haven't opted into this feature see no behavior change. Call once from ClashControl.py's
